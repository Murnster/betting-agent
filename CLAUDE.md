...
Saved to `saved_models/<SPORT>/`: `classifier.json`, `calibrator.joblib`, `home_regression.joblib`, `away_regression.joblib`, `feature_names.pkl`. The calibrator (IsotonicRegression) is trained on a held-out 30% calibration fold during `train_calibrated_classifier()` using a 50/30/20 train/calibration/test split. Probabilities are clipped to [0.05, 0.95] at predict time.
