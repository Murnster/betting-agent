...
Saved to `saved_models/<SPORT>/`: `classifier.json`, `calibrator.joblib`, `home_regression.joblib`, `away_regression.joblib`, `feature_names.pkl`, `scoring_sigma.pkl`, `feature_importance.json`. The calibrator (IsotonicRegression) is trained on a held-out 30% calibration fold during `train_calibrated_classifier()` using a 50/30/20 train/calibration/test split. Probabilities are clipped to [0.05, 0.95] at predict time. `scoring_sigma.pkl` contains `{"total_sigma": float, "margin_sigma": float}` — the empirical standard deviation of regression residuals, computed during training on a held-out 20% temporal split. Used instead of hardcoded sigma for totals/spread normal CDF probability calculations. Totals/spread probabilities are clipped to [0.10, 0.90].
